...
**2. 統一格式整理：**系統會對各格式的成果進行名稱規範化與分類整理，避免寄送內容混亂，使使用者更容易辨識與使用。

**3. 檔案打包壓縮：**所有整合後的成果檔案會由 Compression 節點自動打包成 ZIP，以便於寄送與下載。

**4. 成果寄送與通知：**模組 H 會將ZIP檔寄送至使用者填寫的Email，並附上說明文字與下載連結，讓使用者能快速取得所有最終成果。`,
    },
  ];
  ...
  const systemResultImages = [
    ...
    },
    {
      ...
      title: "自動生成的完整研究報告",
      description:
        "系統自動生成的「Brain Computer Interface」完整PDF報告，包含詳細的技術架構與實現邏輯、競爭策略與差異化價值、問題解決模式與使用者場景等章節內容，並附上完整的參考資料來源。報告格式專業，內容結構清晰，可直接用於學術或商業用途。",
    },
  ];
  ...
  const reportContentImages = [
    {
      ...
      title: "技術架構與類型分析",
      description:
        "詳細分析腦機介面系統的技術架構，涵蓋非侵入性、半侵入性與侵入性等三種主流類型，並探討其訊號擷取方式、資料處理流程與實作挑戰。",
    },
    {
      ...
      title: "神經信號處理與通訊技術比較",
      description:
        "探討腦機介面中訊號濾波、解碼、與通訊協定技術，包含機器學習應用、資料加密、安全傳輸等面向，對比同步與非同步控制系統的差異。",
    },
    {
      ...
      title: "臨床與商業應用案例分析",
      description:
        "彙整腦機介面於神經復健與輔助科技的臨床應用案例，並剖析其在醫療、娛樂、國防與智能家居等商業場域中的實際發展與市場潛力。",
    },
    {
      ...
      title: "未來技術趨勢與倫理議題探討",
      description:
        "探討腦機介面技術未來發展方向，包括AI整合、人機互動優化及開源平台推動，同時深入分析個資保護、認知自主與倫理法規框架等社會挑戰。",
    },
    {
      ...
      title: "延伸閱讀與跨領域資源推薦",
      description:
        "提供與腦機介面相關的延伸閱讀資源，包含醫療應用研究、技術創新論文及倫理挑戰分析，協助深入理解BCI於未來醫療、認知科技與人機互動領域的發展潛力。",
    },
  ];

...
**3. 章節規劃：**Topic Planner（文章架構規劃 Agent）根據 Topic Analyzer 所整理出的資訊，將主題拆解成數個主要章節，並建立文章的大綱結構，包含章節標題與內容方向。`,
    },
    {
      ...
      description: `在文章架構建立後，系統會進入內容撰寫階段。這個階段由 Content Generator 主導撰寫每個章節的內容，而在正式生成段落之前，會先透過底下的工具與 Agent 協助理解資料與補充內容，使生成的文本更完整、更有根據。
      

**1. 關鍵詞查詢與資料擷取：**在開始生成段落內容之前，Content Generator（章節內容撰寫 Agent）會先透過 MCP Tools 查找主題相關的外部資料，包括論文摘要、新聞內容、產業與公司資訊，以及相關技術概念的背景與定義。

**2. 段落內容生成：**在掌握這些資訊後，Content Generator（章節內容撰寫 Agent）會依照章節大綱進行撰寫，將背景脈絡、問題分析、概念比較、實際案例等內容整理成具體且完整的段落，使文章在敘事上更具專業性與邏輯性。

**3. 延伸輔助資料：**Extensive Researcher（延伸內容補強 Agent) 會在 Content Generator 完成初稿後，根據章節內容進行補充查詢，包括延伸研究、相關案例、背景資料等資訊。這些補充資訊會被整理成研究報告後段的「延伸閱讀」區塊，提供讀者在需要時進一步探索主題、取得更多背景脈絡與參考來源。
